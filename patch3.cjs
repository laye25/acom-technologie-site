...
if (newContent === content) {
    console.error("NO CHANGES MADE. Regex mismatch maybe due to spaces.");
    // Try regex
    const regex = /          if \(extractedLayers\.length > 0\) {[\s\S]*?setShowLogoDiagnostic\(true\);\n          }/;
