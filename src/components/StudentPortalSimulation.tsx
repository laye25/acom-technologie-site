...
import {
  ...
  LogOut, Download, AlertTriangle, CheckCircle2, ChevronRight, User, Key, Users, Share2, EyeOff, Beaker, CreditCard,
  Bus, Utensils, DollarSign, Home, Package, Receipt
} from 'lucide-react';
...
export const StudentPortalSimulation = ({ student, merchant, onClose }: { student: any, merchant: any, onClose: () => void }) => {
  ...
  const classSchedules = useMemo(() => {
    ...
    return schedules.filter((s: any) => {
      ...
      const studentClassIdStrTrimmed = (studentClassId || '').toString().trim().toLowerCase();
      const matchesClassId = studentClassIdStrTrimmed && sClassIdStrTrimmed === studentClassIdStrTrimmed;
      ...
      const matchesSGrade = studentGrade && (isClassNameSimilar(sClassIdStr, studentGrade) || isClassNameSimilar(s.grade, studentGrade) || isClassNameSimilar(s.class, studentGrade));
  ...
  const attendances = useMemo(() => {
    ...
    const targetId = studentData.id.toString().trim().toLowerCase();
  ...
  , [resolvedMerchantId]) || [];
  const studentAnnouncements = (communications as any[]).filter(c => c.targetAudience === 'students' || c.targetAudience === 'all');
  ...
  const validHomeworks = useMemo(() => {
    ...
    return homeworks.filter((h: any) => {
      const hClassIdStr = (h.classId || '').toString().trim().toLowerCase();
      const studentClassIdStr = (studentData?.classId || studentData?.class_id || '').toString().trim().toLowerCase();
      const studentClassObjIdStr = (studentClassObj?.id || '').toString().trim().toLowerCase();

      const matchesClassId = studentClassObjIdStr && hClassIdStr === studentClassObjIdStr;
      const matchesStudentClassId = studentClassIdStr && hClassIdStr === studentClassIdStr;
  ...
  const availableGradesSubjects = useMemo(() => {
    ...
    if (studentClassObj && Array.isArray(studentClassObj.subjects)) {
      classSubjectNames = studentClassObj.subjects.map((s: string) => s.trim().toLowerCase());
    }
    ...
    if (rawList.length === 0 && studentGrades) {
      const uniqueIds = Array.from(new Set(studentGrades.map((g: any) => g.subjectId).filter(Boolean)));
  ...
  }, [studentGrades, subjectsMap, studentClassObj, subjects]);
  ...
  const availableGradesTerms = useMemo(() => {
    ...
    } else if (studentGrades && studentGrades.length > 0) {
      rawPeriods = studentGrades.map((g: any) => g.term || 'Trimestre 1');
    } else {
  ...
  const bulletinStats = useMemo(() => {
    ...
    selectedPeriodGrades.forEach((g: any) => {
      ...
      const coef = getSubjectCoef(g.subjectId, studentData?.grade || studentData?.class || '');
  ...
  }, [selectedPeriodGrades, studentData, getSubjectCoef]);
  ...
  const downloadPdf = () => {
    ...
    const sYear = studentData?.schoolYear || "2025-2026";
    doc.setFont("Helvetica", "bold");
    doc.setFontSize(9);
    ...
    doc.text(`${(studentData?.firstName || "").toUpperCase()} ${(studentData?.lastName || "").toUpperCase()}`, 40, 64);
    const mStr = studentData?.matricule || studentData?.id?.substring(0, 8).toUpperCase() || "N/A";
    doc.text(mStr, 45, 70);
    doc.text(`${studentClassObj?.name || studentData?.class || studentData?.grade || "N/A"}`, 40, 76);
    ...
    doc.setFont("Helvetica", "normal");
    doc.setTextColor(15, 23, 42);
    doc.text(`${studentData?.regime || "Externe"}`, 130, 64);
    const bDate = studentData?.birthDate || "---";
    const bPlace = studentData?.birthPlace || "---";
    ...
    const nClean = `${studentData?.lastName || "ELEVE"}_${studentData?.firstName || "PRENOM"}`.toUpperCase().replace(/\s+/g, "_");
  ...
  const financeData = useMemo(() => {
    ...
    const fallback = {
      inscription: 25000, scolarite: 300000, uniforme: 40000,
      transport: 60000, cantine: 90000, internat: 0,
      monthsDuration: 9, hasTransport: true, hasCanteen: true, hasInternat: false
    };
    ...
    const mapConfig = (cfg: any) => {
      ...
      return {
        ...
        hasTransport: hasT, hasCanteen: hasC, hasInternat: hasI,
    ...
    if (pricing?.exceptions?.[st.id]) baseFees = mapConfig(pricing.exceptions[st.id]);
    if (!baseFees) {
      const classKey = st.classId || st.class_id || st.class || st.grade;
      if (classKey && pricing?.classes?.[classKey]) baseFees = mapConfig(pricing.classes[classKey]);
      else {
        const matchedClass = studentClasses.find((c: any) => c.name === classKey || c.id === classKey);
        if (matchedClass && pricing?.classes?.[matchedClass.id]) baseFees = mapConfig(pricing.classes[matchedClass.id]);
        else if (matchedClass && pricing?.classes?.[matchedClass.name]) baseFees = mapConfig(pricing.classes[matchedClass.name]);
      }
    }
    if (!baseFees) {
      ...
      if (levelKey && pricing?.levels?.[levelKey]) baseFees = mapConfig(pricing.levels[levelKey]);
    }

    let hasT = baseFees ? baseFees.hasTransport : fallback.hasTransport;
    if (st.hasTransport !== undefined) hasT = !!st.hasTransport; else if (st.serviceTransport !== undefined) hasT = !!st.serviceTransport;
    let hasC = baseFees ? baseFees.hasCanteen : fallback.hasCanteen;
    if (st.hasCanteen !== undefined) hasC = !!st.hasCanteen; else if (st.serviceCantine !== undefined) hasC = !!st.serviceCantine;
    let hasI = baseFees ? baseFees.hasInternat : fallback.hasInternat;
    if (st.hasInternat !== undefined) hasI = !!st.hasInternat; else if (st.serviceInternat !== undefined) hasI = !!st.serviceInternat; else if (st.regime !== undefined) hasI = (st.regime === 'interne');
    ...
    let finalFees = {
      ...(baseFees || fallback),
      transport: hasT ? rawT : 0, cantine: hasC ? rawC : 0, internat: hasI ? rawI : 0,
      hasTransport: hasT, hasCanteen: hasC, hasInternat: hasI,
    };
    ...
    if (st.tuitionFeesBreakdown && st.tuitionFeesBreakdown.isCustom) {
      ...
      finalFees = {
        ...
        hasTransport: chT, hasCanteen: chC, hasInternat: chI,
  ...
  }, [studentData, merchant, studentClasses]);
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-gray-100">
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-400 mt-0.5">
                    Fiches officielles et relevés périodiques
                  </p>
                </div>
                    ...
                    <div className="border border-slate-300 rounded-3xl p-4 sm:p-8 bg-amber-50/5 shadow-inner max-w-4xl mx-auto animate-fade-in">
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-start gap-4 text-xs font-sans mb-6 text-slate-700">
                        ...
                        <div className="space-y-1 text-left md:text-right font-sans">
                          ...
                          <p className="text-slate-500 text-[11px] font-bold">Année Scolaire : {studentData?.schoolYear || "2025-2026"}</p>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 bg-slate-50/80 rounded-2xl border border-slate-200 mb-6 text-xs text-slate-800">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-black text-indigo-900 uppercase text-left">{studentData?.firstName} {studentData?.lastName}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-slate-400 uppercase w-24 text-left">Matricule :</span>
                            <span className="font-mono font-bold text-slate-800 text-left">{studentData?.matricule || studentData?.id?.substring(0, 8).toUpperCase() || "N/A"}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-slate-400 uppercase w-24 text-left">Classe :</span>
                            <span className="font-black text-emerald-800 uppercase text-left">{studentClassObj?.name || studentData?.class || studentData?.grade || "N/A"}</span>
                          </div>
                        </div>
                        <div className="space-y-2 sm:border-l sm:pl-6 border-slate-200">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-slate-800 text-left">{studentData?.regime || "Externe"}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-slate-400 uppercase w-28 text-left">Né(e) le :</span>
                            <span className="font-bold text-slate-800 text-left">{studentData?.birthDate || "---"} à {studentData?.birthPlace || "---"}</span>
                          </div>
                        </div>
                      </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
              <div>
                <h3 className="text-base sm:text-lg font-black text-slate-900 flex flex-wrap items-center gap-2">
                  Dossier &amp; Historique : <span className="text-indigo-600">{studentData?.firstName} {studentData?.lastName}</span>
                </h3>
                <div className="flex flex-wrap items-center gap-3 mt-1 text-[10px] sm:text-xs text-slate-500 font-mono font-bold">
                  <span>Classe: <span className="text-slate-800 uppercase font-black">{studentData?.grade || studentData?.class || 'N/A'}</span></span>
                  <span>•</span>
                  <span>Matricule: <span className="text-slate-800 font-black">{studentData?.matricule || 'GEN-2026'}</span></span>
                  <span>•</span>
                  <span>Parent: <span className="text-slate-800">{studentData?.parentContact || 'N/A'}</span></span>
                </div>
              </div>
              ...
              <div className="lg:col-span-5 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col h-full self-start">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Consultez tous vos règlements encaissés cette année académique. Vous pouvez afficher et imprimer chaque ticket en un clic.
                  </p>
                </div>
                ...
                <div className="flex-1 mt-6 overflow-y-auto space-y-3 pr-1 max-h-[600px] custom-scrollbar">
                  {(financeData.payments || []).length === 0 ? (
                    <div className="p-12 text-center bg-slate-50 rounded-2xl border border-dashed text-slate-400 text-xs">
                      Aucun versement n'a été enregistré pour le moment.
                    </div>
                  ) : (
                  ...
                  filteredAttendances.map((a: any, idx: number) => (
                    <div key={a.id || idx} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-gray-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-bold text-slate-800 text-sm">{isAbsentStatus(a.status) ? 'Absence Signalée' : 'Retard Noté'}</h4>
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
              ...
              {studentAnnouncements.length === 0 ? (
                ...
                <div className="space-y-4">
                  {studentAnnouncements.map((h: any, idx: number) => (
                    <div key={h.id || idx} className="p-4 bg-slate-50 rounded-2xl border border-indigo-100">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-xs font-black text-indigo-600 uppercase tracking-widest">{h.targetAudience === 'all' ? 'Annonce Globale' : 'Information Élèves'}</span>
            ...
            <div className="bg-gradient-to-r from-indigo-900 to-slate-900 p-6 rounded-[2rem] text-white shadow-md relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-2">
                ...
                <p className="text-xs text-indigo-200 font-medium max-w-xl leading-relaxed">
                  Consultez vos détails opérationnels, vos lignes de bus attribuées, les menus de la cantine et le suivi financier en temps réel.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  {financeData.breakdown?.hasTransport ? (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1">
                          ...
                          <p className="text-xs font-black text-slate-800">{studentData?.transportLine || 'Ligne B - Dakar Plateau / Fann'}</p>
                        </div>
                        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1">
                          <span className="text-[9px] text-gray-400 uppercase font-bold tracking-wider">Chauffeur assigné</span>
                          <p className="text-xs font-black text-slate-800">{studentData?.transportDriver || 'M. Amadou Fall (+221 77 123 45 67)'}</p>
                        </div>
                        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1">
                    ...
                    <div className="py-8 text-center space-y-2">
                      <p className="text-xs text-slate-500 font-medium">Le service de transport scolaire n'est pas activé ou souscrit.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                ...
                {financeData.breakdown?.hasCanteen ? (
                  <div className="space-y-4">
                      ...
                      return (
                        <div className="bg-rose-50/20 p-4 rounded-2xl border border-rose-100/40 space-y-3">
                          <div className="flex justify-between items-center text-xs">
                            ...
                            <span className="font-extrabold text-slate-800 uppercase">{studentData?.regime || 'Demi-pensionnaire'}</span>
                          </div>
                          <div className="flex justify-between items-center text-xs">
                    ...
                    <div className="pt-2">
                      <h5 className="text-[10px] font-mono font-black uppercase text-rose-600 tracking-wider mb-2 flex items-center gap-1">
                        <span>🗓️</span> Menu de la Semaine
                      </h5>
                      <div className="border border-slate-150 rounded-2xl divide-y divide-slate-100 bg-slate-50/30 overflow-hidden">
                        {[
                          ...
                          { day: 'Jeudi', main: 'Spaghetti Bolognaise de bœuf', side: 'Fromage râpé & fruits de saison', meta: 'Carbon' },
                  ...
                  <div className="py-8 text-center space-y-2">
                    <p className="text-xs text-slate-500 font-medium">Le service de cantine scolaire n'est pas activé ou souscrit.</p>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] uppercase font-black tracking-widest text-slate-400">Emploi du Temps</p>
                  <p className="text-[20px] font-black text-slate-800 leading-tight mt-0.5">{classSchedules.length} <span className="text-xs font-bold text-slate-450">cours</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                ...
                <div className="space-y-4">
                  {studentAnnouncements.length === 0 ? (
                    <p className="text-xs text-slate-505 italic py-4">Aucune annonce administrative récente.</p>
                  ) : (
                    studentAnnouncements.slice(0, 3).map((ann: any, idx: number) => (
                      <div key={ann.id || idx} className="flex gap-3 items-start border-b border-gray-50 pb-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-505 mt-1 leading-relaxed max-w-xl whitespace-pre-wrap">{ann.content}</p>
            ...
            {selectedReceipt && (
              <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                <motion.div 
                  ...
                  className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 font-sans text-slate-800"
                >
                  ...
                  <div id="school-receipt-print" className="p-8 space-y-6 printable-body font-sans text-slate-800">
                    ...
                    <div className="py-6 border-y border-dashed border-gray-250 flex justify-between items-center bg-slate-50 border-x rounded-xl px-6 font-sans">
                    ...
                    <div className="flex justify-between items-end gap-4 font-sans">
                      <div className="space-y-1">
                  ...
                  <div className="p-6 bg-slate-50 border-t flex space-x-3 font-sans">
                    <button 
