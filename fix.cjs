...
const search = `          setLayers(prev => {
             const deduplicatedLayers: any[] = [];`;

const replace = `          const deduplicatedLayers: any[] = [];`;

let newContent = content.replace(search, replace);

const search2 = `          if (extractedLayers.length > 0) {
...
const replace2 = `          let finalLayersToSet = deduplicatedLayers;
