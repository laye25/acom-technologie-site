...
// remove everything from the incomplete <input onwards
code = code.replace(/<input[\s\S]*$/i, '');
...
code += `<input
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Prix de reviens par mètre ({merchant.currency})</label>
                  <input
                    type="number"
                    placeholder="0"
                    value={currentTissu.costPricePerMeter || ''}
                    onChange={e => setCurrentTissu({ ...currentTissu, costPricePerMeter: e.target.value })}
                    className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none text-sm font-bold font-mono"
                  />
                  {currentTissu.costPricePerMeter && currentTissu.quantity && (
                     <p className="text-[10px] text-gray-500 mt-1 font-mono">Total Achat : {(Number(currentTissu.costPricePerMeter) * Number(currentTissu.quantity)).toLocaleString()} {merchant.currency}</p>
                  )}
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Prix de vente par mètre ({merchant.currency})</label>
                  <input
                    type="number"
                    placeholder="0"
                    ...
                    className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none text-sm font-bold font-mono"
                  />
                  {currentTissu.pricePerMeter && currentTissu.quantity && (
                     <p className="text-[10px] text-gray-500 mt-1 font-mono">Total Vente : {(Number(currentTissu.pricePerMeter) * Number(currentTissu.quantity)).toLocaleString()} {merchant.currency}</p>
                  )}
                </div>
              </div>
...
`.replace(/merchant\.currency/g, '${merchant.currency}').replace(/currentTissu\.quantity/g, '${currentTissu.quantity}').replace(/currentTissu\.costPricePerMeter/g, '${currentTissu.costPricePerMeter}').replace(/currentTissu\.pricePerMeter/g, '${currentTissu.pricePerMeter}').replace(/currentTissu\.color/g, '${currentTissu.color}').replace(/currentTissu\.supplier/g, '${currentTissu.supplier}').replace(/currentTissu\.name/g, '${currentTissu.name}').replace(/t\.quantity/g, '${t.quantity}').replace(/t\.costPricePerMeter/g, '${t.costPricePerMeter}').replace(/t\.pricePerMeter/g, '${t.pricePerMeter}').replace(/t\.color/g, '${t.color}').replace(/t\.supplier/g, '${t.supplier}').replace(/t\.name/g, '${t.name}').replace(/t\.id/g, '${t.id}');
